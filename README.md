...
프로그램은 루트 디렉터리의 `state.json` 파일을 데이터 저장소로 사용합니다.

#### 💡 JSON(JavaScript Object Notation)이란?
* 데이터를 저장하거나 주고받을 때 사용하는 **텍스트 기반의 경량 데이터 포맷**입니다.
* 사람과 컴퓨터 모두 읽고 쓰기 쉬운 `{ Key : Value }` 구조로 이루어져 있습니다.

#### 💡 JSON 파일을 저장소로 선택한 이유
* **가독성 & 직관성**: 문제, 선택지, 정답이 텍스트 형태(`{ "question": "...", "answer": 1 }`)로 저장되어 데이터를 눈으로 바로 확인하고 수정하기 쉽습니다.
* **Python과의 뛰어난 호환성**: 파이썬의 `dict(딕셔너리)` 및 `list(리스트)` 구조와 1:1로 매핑되어, 기본 `json` 모듈만으로 쉽게 읽고 쓸 수 있습니다.
* **경량성 & 실행 편의성**: 별도의 데이터베이스(MySQL, MongoDB 등)를 설치할 필요가 없어, `git clone` 후 추가 설정 없이 프로젝트를 바로 실행할 수 있습니다.
